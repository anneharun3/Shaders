...
VertexAndFaceArraysPtr vaoFromOBJPath( const std::string& path, const ShaderProgram& program ) {
    ...
    auto flat_Tangent = flatten_attribute( mesh.face_tangents, mesh.tangents );
    vao->uploadAttribute( flat_Tangent, program.getAttribLocation( "vTangent" ) );

    auto flat_Bitangent = flatten_attribute( mesh.face_tangents, mesh.bitangents );
    vao->uploadAttribute( flat_Bitangent, program.getAttribLocation( "vBitangent" ) );
    }
