...
int peekAtTimerMilliseconds( const std::string& scene_path ) {
    ...
    json j;
    const bool success = cs451::loadJSONFromPath( scene_path, j );
    assert( success );
...
void GLGUI::createMenus() {
    ...
    QAction* saveAct = new QAction(tr("&Save Screenshot..."), this);
    saveAct->setShortcuts( QKeySequence::Save );
    connect( saveAct, SIGNAL(triggered()), this, SLOT(saveScreenshot()) );
